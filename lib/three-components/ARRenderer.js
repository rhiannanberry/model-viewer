...
height: 100%;`);
            ...
            this[$outputCanvas].addEventListener('click', () => this[$onOutputCanvasClick]());
        }
        return this[$outputCanvas];
    }
    get outputContext() {
    ...
    async [$onOutputCanvasClick]() {
        ...
        const hits = await this[$currentSession].requestHitTest(originArray, directionArray, this[$frameOfReference]);
        if (hits.length) {
            const hit = hits[0];

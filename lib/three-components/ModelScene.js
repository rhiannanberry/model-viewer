 ...
 */
import { AmbientLight, BackSide, Box3, DirectionalLight, Mesh, MeshBasicMaterial, Object3D, PerspectiveCamera, Scene, SphereBufferGeometry, Vector3 } from 'three';
...
export default class ModelScene extends Scene {
    ...
    constructor({ canvas, element, width, height, renderer }) {
        ...
        const skysphereMat = new MeshBasicMaterial({ side: BackSide, color: 0xffffff, depthTest: false, depthWrite: false });
    ...
    applyRoomSize() {
        ...
        const skysphereSize = Math.max(this.roomSize.x, this.roomSize.y, this.roomSize.z) * 2;
